...
export function PreferenceChart({ preference }: { preference: PreferenceProfile }) {
  ...
  return (
    <View style={theme.apply(styles.preferenceChart, 'preferenceChart')}>
      ...
      {rows.length ? (
        rows.map((row) => {
          const width = 220 * row.value * progress;
          const bar = canRenderSvg() ? (
            <Svg width={220} height={16}>
              <Rect x="0" y="3" width={220} height={10} rx={5} fill={theme.colors.chartTrack} />
              <Rect x="0" y="3" width={width} height={10} rx={5} fill={theme.colors.iconAccent} />
            </Svg>
          ) : (
            <View style={[styles.preferenceChartTrack, { backgroundColor: theme.colors.chartTrack }]}>
